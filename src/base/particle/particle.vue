...
import TweenMax from '@/plugin/TweenMax.min.js'

@Component({
  name: 'particle'
})
class particle extends Vue {
  mounted() {
    this.init()
  }
      ...
      var _size = rand(4, 12), // px width + height
        ...
        _tl = new TimelineMax()
          .fromTo(
            p,
            _dur,
            { x: rand(-_size, cw), y: ch, size: _size },
            {
